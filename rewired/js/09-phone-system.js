...
window.REWIRED.phone = { tab: "messages", thread: null };
...
window.rbPhoneBuildDOM = function () {
	...
	document.getElementById("phone-tabs").addEventListener("click", function (ev) {
		...
		if (!tabEl) { return; }
		window.REWIRED.phone.tab = tabEl.dataset.tab;
		window.REWIRED.phone.thread = null;
		window.rbPhoneRender();
	});
};
...
window.rbPhoneRender = function () {
	...
	tabs.forEach(function (t) {
		t.classList.toggle("active", t.dataset.tab === window.REWIRED.phone.tab);
	});

	var screen = document.getElementById("phone-screen");
	if (!screen) { return; }

	if (window.REWIRED.phone.tab === "messages") {
		screen.innerHTML = window.rbPhoneMessagesView(sv);
	} else if (window.REWIRED.phone.tab === "dating") {
	...
	} else if (window.REWIRED.phone.tab === "social") {
	...
	threadRows.forEach(function (row) {
		row.addEventListener("click", function () {
			window.REWIRED.phone.thread = row.dataset.npc;
			window.rbPhoneRender();
		});
	});
};
...
window.rbPhoneMessagesView = function (sv) {
	...
	var threadId = window.REWIRED.phone.thread;
