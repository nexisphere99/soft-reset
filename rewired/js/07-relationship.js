/* ==========================================================================
   REWIRED   07 RELATIONSHIP / AFFINITY SYSTEM
...
window.REWIRED.mainPartners = ["nolan", "jordan", "sophie", "river", "vanessa"];

/**
 * window.rbAffinityChange(npcKey, amount)
 * npcKey: one of REWIRED.mainPartners, or a supporting-NPC variable name
 * stored directly on State.variables (e.g. "luna" -> $lunaAffinity).
 */
window.rbAffinityChange = function (npcKey, amount) {
	var sv = State.variables;

	if (window.REWIRED.mainPartners.indexOf(npcKey) !== -1) {
		var npc = sv[npcKey];
		if (!npc) { return; }
...
window.rbMeet = function (npcKey) {
	var sv = State.variables;
	if (window.REWIRED.mainPartners.indexOf(npcKey) !== -1 && sv[npcKey]) {
		sv[npcKey].met = true;
	}
};
