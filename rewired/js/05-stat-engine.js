...
window.REWIRED.uncappedStats = ["money"];
...
window.statChange = function (stat, amount, opts) {
	...
	if (window.REWIRED.uncappedStats.indexOf(stat) !== -1) {
...
window.showStatNotification = function (stat, amount) {
	try {
		var colors = window.REWIRED.statColors || {};
		var names = window.REWIRED.statNames || {};
		var sign = amount > 0 ? "+" : "";
		var arrow = amount > 0 ? "▲" : "▼";
		...
		console.warn("REWIRED: notification failed", e);
	}
};
