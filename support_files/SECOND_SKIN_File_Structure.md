# SECOND SKIN   Twine File Structure
...
**On disk, `assets/` and the compiled HTML live *outside* `second-skin/`, as
...
```
second_skin/                      ← project root
├── assets/                  ← sibling of second-skin/, see the tree below
├── second-skin-compiled.html     ← build output, sibling of assets/
└── second-skin/                  ← everything below is inside THIS folder
```

The rest of this document (and the tree below) describes what's inside
`second-skin/`   read every path from here on as relative to that folder,
with `assets/` and the compiled HTML understood to live one level up.

```
second-skin/
├── second-skin.twee              ← Main entry: StoryTitle, StoryData, StoryInit, StoryCaption
├── compile.sh                ← Build script (tweego compilation; run from inside second-skin/, writes ../second-skin-compiled.html)
├── css/
...
│   │   └── d01-night.twee       ← Alone at home, Nolan's call, bed
│   │
│   ├── locations/
...
│   │   ├── thriftstore.twee     ← Thrift store (cheap clothes, Marlene)
...
│   │   ├── tattoo-shop.twee     ← Black Line Studio (Jordan)
...
│   │   ├── foundry.twee         ← BDSM club (Jordan route)
...
│   │   │   ├── derek.twee           ← Owen (DashDrop manager)
│   │   │   ├── dolores.twee         ← Marlene (thrift store)
...
│   │   │   └── benny.twee           ← Benny (Nolan's friend)
│   │   │
│   │   └── random/
...
`assets/` is **not** inside `second-skin/`   see the project-root tree at the
...
assets/
├── ui/
│   ├── second-skin-logo.png
│   └── icons/              ← UI icons (stat icons, nav icons)
├── characters/
│   ├── mc-male/            ← Marcus images (Day 0)
│   ├── mc-female-phase1/   ← Mia B-cup images
│   ├── mc-female-phase2/   ← Mia C-cup images
│   ├── mc-female-phase3/   ← Mia D-cup images
│   ├── mc-female-phase4/   ← Mia DD-cup images
│   ├── jake/
│   ├── marcus/
│   ├── sophie/
│   ├── river/
│   ├── vanessa/
│   └── supporting/         ← Luna, Owen, Marlene, etc
├── locations/
│   ├── apartment/
│   ├── campus/
│   ├── novacure/
...
### 9. Compilation
Using **Tweego** CLI compiler, run from inside `second-skin/` so the output
lands one level up, next to `assets/`:
```bash
cd second-skin/
tweego -o ../second-skin-compiled.html \
  second-skin.twee \
  css/*.css \
  js/*.js \
  passages/**/*.twee \
...
Or just run `second-skin/compile.sh`, which does exactly this. Or for
development with live reload:
```bash
tweego -o ../second-skin-dev.html -w \
  second-skin.twee css/ js/ passages/
```
...
### Player Stats
```
$player.name         : ""        ← Player-chosen female name (default "Mia")
$player.maleName     : "Marcus"   ← Original male name
...
$metMarlene      = false
$metLuna         = false
$metOwen        = true     ← Known from DashDrop
$metDrAmelia       = false
$metNursePatel   = false
$metTyler        = false
$tylerNumber     = false
...
All commands below run from inside `second-skin/`, and all write their output
...
### Production Build
```bash
tweego -o ../second-skin-v1.0.html \
...
*This document defines the complete file architecture for SECOND SKIN. All content from the 9-file Day-by-Day Summary maps directly to this structure. Ready for file creation.*
