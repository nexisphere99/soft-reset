...
**On disk, `assets/` and the compiled HTML live *outside* `rewired/`, as
...
```
rewired/                      ← project root
├── assets/                  ← sibling of rewired/, see the tree below
├── rewired-compiled.html     ← build output, sibling of assets/
└── rewired/                  ← everything below is inside THIS folder
```

The rest of this document (and the tree below) describes what's inside
`rewired/`   read every path from here on as relative to that folder,
with `assets/` and the compiled HTML understood to live one level up.

```
rewired/
├── rewired.twee              ← Main entry: StoryTitle, StoryData, StoryInit, StoryCaption
├── compile.sh                ← Build script (tweego compilation; run from inside rewired/, writes ../rewired-compiled.html)
├── css/
...
`assets/` is **not** inside `rewired/`   see the project-root tree at the
...
### 9. Compilation
Using **Tweego** CLI compiler, run from inside `rewired/` so the output
lands one level up, next to `assets/`:
```bash
cd rewired/
tweego -o ../rewired-compiled.html \
  rewired.twee \
  css/*.css \
  js/*.js \
  passages/**/*.twee \
...
Or just run `rewired/compile.sh`, which does exactly this. Or for
development with live reload:
```bash
tweego -o ../rewired-dev.html -w \
  rewired.twee css/ js/ passages/
```
...
All commands below run from inside `rewired/`, and all write their output
...
*This document defines the complete file architecture for REWIRED. All content from the 9-file Day-by-Day Summary maps directly to this structure. Ready for file creation.*
