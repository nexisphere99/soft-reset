# PROMPT FOR CODE AGENT — Add Day 2 to SECOND SKIN (Open World Architecture)
...
Players DISCOVER content by exploring locations. They are never railroaded from Scene A → Scene B. Instead, they see "You need underwear" as an objective, see "Thrift Store" as an available location, go there, and the Marlene scene plays. Afterward they're back at the hub, not auto-forwarded to the catcall. The catcall happens DURING the walk home (a transit event that fires based on flags, not because it's "next in sequence").

---

## CONTEXT

You have built Days 0–1 of SECOND SKIN. Day 0 was linear (tutorial day — acceptable). Day 1 was mostly linear (crisis day — acceptable). Starting Day 2, the game transitions into its open-world structure.
...
/* This gets pushed when $metMarlene becomes true */
...
<<if not $metMarlene>>
  /* FIRST VISIT: Full Marlene scene from content file */
  /* (Marlene intro, shopping, buying, $32 deduction, wardrobe items added) */
...
<<else>>
  /* REPEAT VISIT: Browse, chat with Marlene */
  <div class="prose-block">
    I push open the door. The bell chimes. Marlene looks up from behind the counter. "Back already, honey?"
...
<div class="choice-block">
  <<link '<span>🗺️ Return to map</span>' "LocationHub">>
    ...
    <<if $metMarlene and not $firstCatcall>>
...
<<if not $d02TriedOnClothes and $metMarlene>>
...
<<if $timeslot is "Evening" or $timeslot is "Night">>
  <<if not $metCody>>
    /* First time ordering — Cody NPC encounter */
    /* (Full Cody scene from content file) */
    <<set $metCody = true>>
    <<set $player.money -= 26>>
    <<run completeObjective("d02_eat")>>
  <<else>>
    /* Repeat order — Cody may or may not deliver */
    <<set $player.money -= 20>>
    <<run completeObjective("d02_eat")>>
  <</if>>
<<else>>
...
/* Nolan's message — always on Day 2 */
<<if $day is 2 and not $d02NolanTexted>>
  <<Dialogue "Nolan" "💪" "seriously u good?? Can I do anything?" "">>

  <div class="choice-block">
    <<link '<span>💬 "yeah just a bug. Give me a couple days."</span>' "Apt_Phone_After">>
      <<set $d02NolanTexted = true>>
    <</link>>
  </div>
<<else>>
...
<<Dialogue "Nolan" "💪" "aight feel better dude 💪" "">>

<div class="prose-block">
  Dude. He called me dude. I put the phone face-down on the couch.
</div>

/* University email — appears after Nolan text */
<<if not $d02UniEmail>>
  <div class="prose-block">
    One more notification. Email from the university: "Please confirm your preferred name for academic correspondence."
    
    I type "<<print $player.name>>." Stare at it. Delete it. Type "Marcus." Stare at that. Delete it. Type "<<print $player.name>>" again.
...
| `Loc_ThriftStore` | **Location** | Marlene scene (first visit) / Shop browse (repeat) |
...
| `Apt_Phone` | **Apartment action** | Nolan text, university email |
| `Apt_Phone_After` | **Apartment action** | Nolan reply + uni email confirmation |
| `Apt_OrderFood` | **Apartment action** | Order delivery — Cody NPC (evening/night only) |
...
- [ ] Cody delivery NPC only appears during Evening/Night timeslot
- [ ] Phone shows Nolan text and university email with proper conditionals
