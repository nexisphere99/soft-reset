# PROMPT FOR WRITING/CODE AGENT   Add Day 1 to SECOND SKIN (Open-World Navigation)

---

## CONTEXT   READ THIS FIRST

Day 0 of SECOND SKIN was originally built as a **linear, scene-to-scene** Twine game (click "Continue" → next passage). It has since been **rebuilt as an open-world hub**, and Day 1 must follow the same pattern. Do not build Day 1 as a chain of "continue" links.
...
- **`Outside` hub** (`passages/locations/neighborhood.twee`)   a `.location-grid` map: DashDrop Zone, Campus, Corner Store, Thrift Store, Meridian Tower (NovaCure), Nolan's Apartment. Cards are either clickable or **locked with a visible reason** (`.loc-card.locked` + `.loc-lock-req` text), driven by story flags.
...
- **`SECOND_SKIN_Day1_Content_FirstPerson.md`**   the complete, already-written Day 1 prose (~8,200 words, 8 scenes + 2 branch scenes). The words are good and mostly reusable as-is. **What needs to change is delivery, not prose**   you are re-plumbing how the player reaches each scene, not rewriting the scenes themselves.
- **`SECOND_SKIN_Day1_Code_Agent_Prompt.md`**   the OLD linear implementation brief. Its stat/flag tables (section 4) and StoryInit additions (section 5) are still valid and should be reused. **Ignore its passage-flow diagram and "NEW PASSAGES TO CREATE" table   that's the linear structure you're replacing.**
...
| `D01_EveningHome` (ramen, Nolan's call) | `Home_Kitchen`   extend its existing "Eat breakfast"-style one-time action pattern with a Day-1 dinner variant |
...
- Don't lose any prose from `SECOND_SKIN_Day1_Content_FirstPerson.md`   every scene should still be reachable and complete, just entered from a location card or room action instead of a "Continue" link.
...
- [ ] `Home_Kitchen` dinner beat (ramen, Nolan's call) fires once, correctly gated
...
- Adds all Day 1 prose from `SECOND_SKIN_Day1_Content_FirstPerson.md`, entered entirely through hub/location navigation rather than direct scene links (except the unavoidable linear cold-open)
