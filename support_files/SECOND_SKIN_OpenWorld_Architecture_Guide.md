# SECOND SKIN   Open-World Navigation Architecture Guide

*Reference this document whenever writing a code-agent prompt for any day of SECOND SKIN (Day 1 through Day 90). It doesn't change per day   only the content being slotted into it does.*

---

## THE CORE PRINCIPLE

**SECOND SKIN is an open-world life-sim, not a visual novel.** The player is always standing somewhere. The story does not advance because the player clicked "Continue"   it advances because the player walked to the right place at the right time.
...
- **`Outside`** (`passages/locations/neighborhood.twee`)   a `.location-grid` map of every place in the world the player can currently reach: DashDrop Zone, Campus, Corner Store, Thrift Store, Meridian Tower, Nolan's Apartment, and whatever new locations get added over the game's 90 days.
...
> Write a code-agent implementation prompt for **Day [N]** of SECOND SKIN.
>
> Follow the architecture in `SECOND_SKIN_OpenWorld_Architecture_Guide.md` exactly   Day [N] must be implemented as logic- and objective-driven hub navigation, not a linear scene chain, per that guide's rules.
>
