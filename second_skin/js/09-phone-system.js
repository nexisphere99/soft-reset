...
window.SECOND_SKIN.phone = { tab: "messages", thread: null };
...
window.rbPhoneBuildDOM = function () {
	...
	document.getElementById("phone-tabs").addEventListener("click", function (ev) {
		...
		if (!tabEl) { return; }
		window.SECOND_SKIN.phone.tab = tabEl.dataset.tab;
		window.SECOND_SKIN.phone.thread = null;
		window.rbPhoneRender();
	});
};
...
window.rbPhoneRender = function () {
	...
	tabs.forEach(function (t) {
		t.classList.toggle("active", t.dataset.tab === window.SECOND_SKIN.phone.tab);
	});

	var screen = document.getElementById("phone-screen");
	if (!screen) { return; }

	if (window.SECOND_SKIN.phone.tab === "messages") {
		screen.innerHTML = window.rbPhoneMessagesView(sv);
	} else if (window.SECOND_SKIN.phone.tab === "dating") {
	...
	} else if (window.SECOND_SKIN.phone.tab === "social") {
	...
	threadRows.forEach(function (row) {
		row.addEventListener("click", function () {
			window.SECOND_SKIN.phone.thread = row.dataset.npc;
			window.rbPhoneRender();
		});
	});
};
...
window.rbPhoneMessagesView = function (sv) {
	...
	var threadId = window.SECOND_SKIN.phone.thread;
