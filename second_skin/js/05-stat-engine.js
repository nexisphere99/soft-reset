...
window.SECOND_SKIN.uncappedStats = ["money"];
...
window.statChange = function (stat, amount, opts) {
	...
	if (window.SECOND_SKIN.uncappedStats.indexOf(stat) !== -1) {
...
window.showStatNotification = function (stat, amount) {
	try {
		var colors = window.SECOND_SKIN.statColors || {};
		var names = window.SECOND_SKIN.statNames || {};
		var sign = amount > 0 ? "+" : "";
		var arrow = amount > 0 ? "▲" : "▼";
		...
		console.warn("SECOND_SKIN: notification failed", e);
	}
};
