/* ==========================================================================
   SECOND_SKIN   07 RELATIONSHIP / AFFINITY SYSTEM
...
window.SECOND_SKIN.mainPartners = ["nolan", "jordan", "sophie", "river", "vanessa"];

/**
 * window.rbAffinityChange(npcKey, amount)
 * npcKey: one of SECOND_SKIN.mainPartners, or a supporting-NPC variable name
 * stored directly on State.variables (e.g. "luna" -> $lunaAffinity).
 */
window.rbAffinityChange = function (npcKey, amount) {
	var sv = State.variables;

	if (window.SECOND_SKIN.mainPartners.indexOf(npcKey) !== -1) {
		var npc = sv[npcKey];
		if (!npc) { return; }
...
window.rbMeet = function (npcKey) {
	var sv = State.variables;
	if (window.SECOND_SKIN.mainPartners.indexOf(npcKey) !== -1 && sv[npcKey]) {
		sv[npcKey].met = true;
	}
};
